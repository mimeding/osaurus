    ...
    @Test func workspaceSummaryAppearsAsDynamicPromptSection() async throws {
        try await SandboxTestLock.runWithStoragePaths {
            ...
            do {
                ...
                #expect(context.prompt.contains("Context source: agentWorkspace"))
                #expect(context.prompt.contains("Owner: agent"))
                #expect(context.prompt.contains("Provenance: Durable workspace metadata"))
                #expect(context.prompt.contains("Injection slot: systemPromptDynamic"))
                #expect(context.prompt.contains("Privacy path: localSystemPrompt"))
                #expect(context.prompt.contains("Boundary: these are durable knowledge-base summaries"))
                ...
                #expect(context.prompt.contains("[Memory]") == false)
                #expect(context.prompt.contains("[Screen Context]") == false)
                #expect(context.prompt.contains("## Working directory") == false)
                #expect(context.prompt.contains("## Linux sandbox environment") == false)
                #expect(context.prompt.contains("## Sandbox state") == false)
