...
//  `invoke(type:id:payload:)` callback. Created at registration time by
//  `PluginHostAPI.registerParser(requestJSON:)` / `registerEmitter(requestJSON:)`
//  when a plugin calls the corresponding host callbacks from its `init`
//  entry point.
//
